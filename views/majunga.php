<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="css/majunga.css">
    <title>Document</title>
    <link rel="stylesheet" href="bootstrap/css/bootstrap.css">
</head>
<body>
<div class="majunga">
  <div class="main" id="main1">
  <img src="kk.png" alt="">
  <hr>
  <p>
  Le Baobab Tree est un hôtel 4 étoiles se situant dans le centre de Majunga,à quelques mètres du célèbre bord de Majunga et du Grand Baobab.Avec 48 chambres et 4 Suites ,l'équipe du Baobab Tree s'éfforce de répondre aux besoins particuliers de ses hôtes ,en voyage d'affaires ou en vacances dans la région. 
  </p>

  <!-- Portfolio Gallery Grid -->
  <div class="row">
    <div class="column">
      <div class="content">

        <img src="photo/majunga_photo/kalymaraina.jpg" alt="Mountains" style="width:100%">
        <h3>Le petit-déjeuner</h3>
        <p>Un vrai régal <a href="photo/majunga_photo/kalymaraina.jpg">voir</a></p>

      </div>
    </div>
    <div class="column">
      <div class="content">
        <img src="photo/majunga_photo/massaage.jpg" alt="Lights" style="width:100%">
        <h3>Spa</h3>
        <p>Relaxation au maximum <a href="photo/majunga_photo/massaage.jpg">voir</a></p>

      </div>
    </div>
    <div class="column">
      <div class="content">
        <img src="photo/majunga_photo/chambre b.jpg" alt="Nature" style="width:100%">
        <h3>Chambre de luxe</h3>
        <p>Le confort est à porter de mains <a href="photo/majunga_photo/chambre b.jpg">voir</a></p>

      </div>
    </div>
    <div class="column">
      <div class="content">
        <img src="photo/majunga_photo/piscine.jpg" alt="Mountains" style="width:100%">
        <h3>La piscine</h3>
        <p>Avec une vue à couper de souffle <a href="photo/majunga_photo/piscine.jpg">voir</a></p>
      </div>
    </div>
  </div>

  <div class="content">
    <img src="photo/majunga_photo/shop.png" alt="Bear" style="width:100%">
    <h3>BAOBAB TREE HOTEL &#38; SPA  <a href="photo/majunga_photo/shop.png">voir Logo</a></h3>
    <form action="index.php?action=Baobaboffre" method="post">
      <input type="submit" value="voir les offres" class="btn btn-primary">
    </form>
  </div>
  <br>
  <!-- END MAIN -->
  </div>
  <div class="main" id="main1">
  <img src="kk.png" alt="">
  <hr>
  <p>
    Situé à Majunga ,à moins de 100 mètres de la plage de Soma ,le PAP'S Chambre d'Hôtes propose des hébergements dotés d'une connexion Wi-Fi gratuite.Chaque chambre de l'établissement 2 étoiles est pourvue de tout pour subvenir à vos besoins quotidiens avec un paysage qui sort de l'ordinaire. 
  </p>

  <!-- Portfolio Gallery Grid -->
  <div class="row">
    <div class="column">
      <div class="content">
        <img src="photo/majunga_photo/ppd.jpg" alt="Mountains" style="width:100%">
        <h3>Le petit-déjeuner</h3>
        <p>Tous simplement divin <a href="photo/majunga_photo/ppd.jpg">voir</a></p>
      </div>
    </div>
    <div class="column">
      <div class="content">

        <img src="photo/majunga_photo/retsoration paps.jpg" alt="Lights" style="width:100%">
        <h3>Restauration</h3>
        <p>Restaurant de qualité avec des plats savoureux <a href="photo/majunga_photo/retsoration paps.jpg">voir</a></p>
      </div>
    </div>
    <div class="column">
      <div class="content">
        <img src="photo/majunga_photo/ch paps.jpg" alt="Nature" style="width:100%">
        <h3>Chambre de luxe</h3>
        <p>Mettez-vous à votre aise <a href="photo/majunga_photo/ch paps.jpg">voir</a></p>

      </div>
    </div>
    <div class="column">
      <div class="content">
        <img src="photo/majunga_photo/pisc paps.jpg" alt="Mountains" style="width:100%">
        <h3>La piscine</h3>
        <p>Piscine privée et un paysage splendide <a href="photo/majunga_photo/pisc paps.jpg">voir</a></p>
      </div>
    </div>
  </div>

  <div class="content">

    <img src="photo/majunga_photo/logo psp.png" alt="Bear" style="width:100%">
    <h3>PAP'S CHAMBRE D'HÔTES <a href="photo/majunga_photo/logo psp.png">voir Logo</a></h3>
    <form action="index.php?action=Papsoffre" method="post">
      <input type="submit" value="voir les offres" class="btn btn-primary">
  </form>
  </div>
<br>
  <!-- END MAIN -->
  </div>
  <div class="main">
  <img src="kk.png" alt="">
  <hr>
  <p>
    Situé non loin de la ville de Majunga ,au bord de la rivière Morira, l'hôtel Antsanitia Resort domine tout l' estuaire et offre une vue extraordinaire sur l'océan Indien et le canal du Mozambique.Niché dans un grand jardin tropical,l'hôtel propose un hébergement de 33 unités différentes .Toutes parfaitements conçues pour vous stisfaire .
  </p>

  <!-- Portfolio Gallery Grid -->
  <div class="row">
    <div class="column">
      <div class="content">
        <img src="photo/majunga_photo/Resto-ATS-101.png" alt="Mountains" style="width:100%">
        <h3>Restauration</h3>
        <p>Restaurant luxueux <a href="photo/majunga_photo/Resto-ATS-101.png">voir</a></p>

      </div>
    </div>
    <div class="column">
      <div class="content">

        <img src="photo/majunga_photo/plage antsa.jpg" alt="Lights" style="width:100%">
        <h3>Plage</h3>
        <p>La meilleure de toutes <a href="photo/majunga_photo/plage antsa.jpg">voir</a></p>
      </div>
    </div>
    <div class="column">
      <div class="content">

        <img src="photo/majunga_photo/ch ants.jpg" alt="Nature" style="width:100%">
        <h3>Vos chambres</h3>
        <p>A la traditionnelle  <a href="photo/majunga_photo/ch ants.jpg">voir</a></p>

      </div>
    </div>
    <div class="column">
      <div class="content">

        <img src="photo/majunga_photo/ants pisc.jfif" alt="Mountains" style="width:100%">
        <h3>Grande piscine</h3>
        <p>La soeur d'Eden <a href="photo/majunga_photo/ants pisc.jfif">Voir</a></p>

      </div>
    </div>
  </div>

  <div class="content">
    <img src="photo/majunga_photo/LOGO antsa.png" alt="Bear" style="width:100%">
    <h3>ANTSANITIA RESORT <a href="photo/majunga_photo/LOGO antsa.png">voir logo</a></h3>
    <form action="index.php?action=Antsanitiaoffre" method="post">
      <input type="submit" value="voir les offres" class="btn btn-primary">
  </form>
  </div>

  <!-- END MAIN -->
  </div>
 </div>

</body>
</html>